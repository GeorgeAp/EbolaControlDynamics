function y = ebola_opt_vac_only_ctl(beta,epsilon,delta,rho,alpha,omega,sigma,omegaI,T,S0,E0,I0,R0,c1)
...
%state equations
    for i = 1:M
        ...
        m3=delta*E(i)-(rho+omegaI)*I(i);
        %m4=u2(i)*I(i)-meu*Q(i)-omegaQ*Q(i);
        m5=rho*I(i)-alpha*R(i)+u1(i)*E(i)+u1(i)*S(i);
        S(i+1)=S(i)+h*m1;
        E(i+1)=E(i)+h*m2;
        I(i+1)=I(i)+h*m3;
        %Q(i+1)=Q(i)+h*m4;
    ...
    for i=1:M
        ...
        m3=-1+beta*S(j+1)*p1(j+1)-beta*S(j+1)*p2(j+1)+p3(j+1)*rho+p3(j+1)*omegaI-p5(j+1)*rho;
