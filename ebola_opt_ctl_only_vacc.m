function y = ebola_opt_ctl_only_vacc(beta,epsilon,delta,rho,meu,alpha,omega,sigma,omegaI,omegaQ,T,S0,E0,I0,Q0,R0,c1)
...
%state equations
    ...
    for i=1:M
        j=M+1-i;
        m1=-1+p1(j+1)*beta*I(j+1)+p1(j+1)*epsilon*R(j+1)+omega*p1(j+1)+u1(j+1)*p1(j+1)-p2(j+1)*beta*I(j+1)-p2(j+1)*epsilon*R(j+1)-p5(j+1)*u1(j+1);
        m2=-1+delta*p2(j+1)+omega*p2(j+1)+p2(j+1)*u1(j+1)-p3(j+1)*delta-p5(j+1)*u1(j+1);
